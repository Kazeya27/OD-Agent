        ...
        with open(input_file_path, mode='r', encoding='utf-8') as f:
            ...
            for row in reader:
                try:
                    ...
                    coords_str = row['coordinates'].strip().replace('[', '').replace(']', '').replace(' ', '')
...
def get_driving_distance(origin_coords, dest_coords, api_key):
    ...
    params = {'key': api_key, 'origin': origin_coords, 'destination': dest_coords, 'extensions': 'base'}
    try:
...
def process_pair(origin_id, dest_id, city_data, api_key, cache_dir):
    ...
    if origin_id != dest_id:
        cost = None
        while not cost:
            cost = get_driving_distance(origin['coords'], destination['coords'], api_key)
        time.sleep(0.05)
    ...
    try:
        with open(output_file, 'w', encoding='utf-8', newline='') as f_out:
            ...
            for filename in tqdm(cache_files, desc="Consolidating", unit="file"):
                try:
                    with open(os.path.join(cache_dir, filename), 'r', encoding='utf-8') as f_in:
                        content = f_in.read().strip()
                        if not content: continue
                        origin_id, dest_id, cost = content.split(',')
                        
                        writer.writerow([rel_id_counter, 'geo', origin_id, dest_id, cost])
                        rel_id_counter += 1
                except Exception as e:
...
def main():
    ...
    parser.add_argument("--dataset", default="baidu", help="数据集名称，用于构建输入/输出路径。")
    parser.add_argument("--api_key", default="30d31b7586b104cf0eeaef5105a1e912", help="您的高德地图 Web 服务 API Key。")
    parser.add_argument("-t", "--threads", type=int, default=16, help="使用的线程数量。")
    ...
    with ThreadPoolExecutor(max_workers=args.threads) as executor:
        # 提交所有任务，让executor管理
        futures = [executor.submit(process_pair, origin_id, dest_id, city_data, args.api_key, cache_dir) 
                   for origin_id, dest_id in all_pairs]
        
        # 使用tqdm来显示已完成任务的进度
        progress_bar = tqdm(as_completed(futures), total=total_tasks, desc="Executing Tasks")
