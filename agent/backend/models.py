...
class ProvinceFlowRequest(BaseModel):
    ...
    period_type: str = Field(
        ...,
        description="时期类型：如 'daily'（日常）、'2026_spring_festival'（26年春运）等",
    )
    start: str = Field(..., description="起始时间（ISO8601，如 2022-01-11T00:00:00Z）")
    end: str = Field(..., description="结束时间（ISO8601，不包含该时刻）")
    date_mode: DateMode = Field(
        default=DateMode.daily,
        description="时间维度：'daily'（每日）或 'total'（假期总量）",
    )
    direction: Direction = Field(
        default=Direction.send,
        description="计算维度：'send'（发送）或 'receive'（接收）",
    )
    dyna_type: Optional[str] = Field(
        default=None, description="按 dyna.type 过滤（可选）"
    )
...
class ProvinceFlowRecord(BaseModel):
    ...
    date: Optional[str] = Field(
        None, description="日期（date_mode='daily'时有值，'total'时为空）"
    )
...
class ProvinceCorridorRequest(BaseModel):
    ...
    date_mode: DateMode = Field(
        default=DateMode.total, description="时间维度（建议使用'total'）"
    )
...
class CityCorridorRecord(BaseModel):
    ...
    corridor_type: str = Field(
        ..., description="通道类型：'intra_province'（省内）或 'inter_province'（省际）"
    )
